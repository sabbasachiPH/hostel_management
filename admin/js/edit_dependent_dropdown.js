// http://localhost/hostel_group2/admin/edit_api_data.php/?type=froom&floor_id=1
...
async function get_hall() {
  const response = await fetch(
    "http://localhost/hms_38/admin/edit_edit_api_data.php"
  );
...
async function get_floor(hall_id) {
  const response = await fetch(
    "http://localhost/hms_38/admin/edit_api_data.php/?type=floor&hall_id=" +
      hall_id
  );
  const json_data = await response.json();
  ...
  let selected = "";
  json_data.forEach((item, index) => {
    if (item.floor_id == hall_dropdown.value) {
      selected = "selected";
    }
    const option = `<option selected='' value="${item.floor_id}">${item.floor_name}</option>`;
...
if (room_dropdown != null) {
  async function get_room(floor_id) {
    ...
    const response = await fetch(
      "http://localhost/hms_38/admin/edit_api_data.php/?type=froom&floor_id=" +
        floor_id
    );
    const json_data = await response.json();
